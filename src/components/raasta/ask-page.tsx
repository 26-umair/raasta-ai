...
import { analysisSteps, exampleScenarios, fieldsForScenario, mockScenarios, type MockScenarioKey } from "./data";
...
export function AskRaastaPage() {
  ...
  const finish = useCallback(() => {
    ...
    navigate({ to: "/recommendation", search: { flexible: urgency === "flexible" ? "yes" : undefined } });
  ...
  return <div className="page-shell ask-page"><header className="ask-heading"><p className="eyebrow"><Wand2 /> AI Payment Advisor for Pakistan</p><h1>How should you receive this payment in Pakistan?</h1><p>Tell Raasta about your client, payment and priorities. We'll compare the routes that actually fit your situation.</p></header>
    {stage === "prompt" ? <section className="prompt-section" aria-label="Describe your payment"><div className="prompt-box"><Textarea aria-label="Payment situation" value={prompt} onChange={(event) => { setPrompt(event.target.value); setScenario("golden"); }} rows={7} /><div className="prompt-footer"><span><ShieldCheckIcon /> Uses mock rules for this prototype</span><Button size="lg" onClick={() => setStage("context")} disabled={!prompt.trim()}>Find My Best Route <ArrowRight /></Button></div></div><div className="example-row" aria-label="Example prompts">{exampleScenarios.map((example) => <button type="button" key={example.key} onClick={() => selectScenario(example.key)}>{example.label}</button>)}</div><aside className="principle-note"><span>Good to know</span><p>The cheapest route is not always the best route. Timing, payment records and how you use your earnings can matter too.</p></aside></section>

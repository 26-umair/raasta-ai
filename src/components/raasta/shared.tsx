...
export function StatusBadge({
  ...
  const badge = (
    <span className={cn("status-badge", item.className)}>
      <item.Icon />
      {label ?? item.label}
    </span>
  );
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button
            ...
            aria-label={`${label ?? item.label}: ${help}`}
          >
            {badge}
          </button>
        </TooltipTrigger>
...
export function LoadingSequence({
  ...
  return (
    <div className="loading-sequence" aria-live="polite">
      ...
      <div>
        <p className="eyebrow">Raasta is reasoning</p>
        <h2>{steps[active]}</h2>

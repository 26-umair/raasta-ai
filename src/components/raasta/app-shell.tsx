...
function SourceEvidence({ onClose }: { onClose?: () => void }) {
  return (
    <div className="drawer-stack">
      ...
      ].map(([name, topic, status, date]) => (
        <article className="evidence-item" key={name}>
          <div>
            <span className="evidence-source">{name}</span>
            <StatusBadge
              type={
                status === "Verified" ? "verified" : status === "May change" ? "estimated" : "needs"
              }
              label={status}
            />
          </div>
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("raasta:open-sources", openSources);
  ...
  const navigation = (
    <>
      ...
      <nav aria-label="Primary navigation" className="side-nav">
        {nav.map(({ to, label, Icon }) => (
          <Link
            ...
            className={cn("side-link", path === to && "active")}
          >
            <Icon />
            {label}
          </Link>
      ...
      <button className="verified-box" type="button" onClick={() => setSourcesOpen(true)}>
        <span>
          <ShieldCheck />
          Rules verified
        </span>
        <strong>12 Sep 2026</strong>
        <small>SBP · FBR · PSEB</small>
      </button>
    </>
  );
  return (
    <PaymentContextProvider>
      <div className="app-shell">
        ...
        <Sheet open={sourcesOpen} onOpenChange={setSourcesOpen}>
          <SheetContent className="evidence-drawer">
            <SheetHeader>
              <SheetTitle>Rule Evidence</SheetTitle>
              <SheetDescription>
                What currently supports Raasta's Pakistan-specific context.
              </SheetDescription>
            </SheetHeader>
            <SourceEvidence onClose={() => setSourcesOpen(false)} />
          </SheetContent>
        </Sheet>
      </div>
    </PaymentContextProvider>
